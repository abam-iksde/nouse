...
namespace nouse {
	...
	void Context::newObject(Object* o) {
		this->objects.push_back(o);
	}
	void Context::deleteObject(Object* o) {
		std::map<stdstr_t, Value*>::iterator it;
		for (it = this->globalVariables->fields.begin(); it != this->globalVariables->fields.end(); it++) {
		...
		for (i64 i = 0; i < this->variables.size(); i++) {
			for (i64 j = 0; j < this->variables[i].size(); j++) {
				for (it = this->variables[i][j]->fields.begin(); it != this->variables[i][j]->fields.end(); it++) {
					if (it->second->getType() == ValueType::OBJECT) {
						if (it->second->getObject() == o) {
							it->second->makeNone();
						}
	...
	Value* Context::getVariable(stdstr_t name, i64 branch) {
		//std::cout << "looking for " << name << " on branch " << branch << std::endl;
		...
		if (branch != 0) {
			for (i64 i = 0; i < this->variables[0].size(); i++) {
				if (this->variables[0][i]->hasField(name)) return this->variables[0][i]->getField(name);
			}
		}
		if (this->globalVariables->hasField(name)) return this->globalVariables->getField(name);
	...
	Value* Context::getVariableCurrentScope(stdstr_t name, i64 branch) {
		...
		return this->variables[branch][this->variables[branch].size()-1]->getField(name);
	}
	...
	void Context::popBranch() {
		for (i64 i = 0; i < this->variables[this->variables.size() - 1].size(); i++) {
			if (this->scopeDisposable[this->variables.size() - 1][i]) delete this->variables[this->variables.size() - 1][i];
		}
	...
	void Context::branchPopScope(i64 branch) {
		if (this->scopeDisposable[branch][this->variables[branch].size() - 1]) delete this->variables[branch][this->variables[branch].size() - 1];
	...
	void Context::registerFunction(stdstr_t name, Value* (*f)(Context* ctx,i64 branch,i64 line, i64 fileind),boolean resultDisposable) {
	...
	void Context::trackFunction(Function* f) {
	...
	void Context::popFunctionArgs() {
		this->functionArguments.pop_back();
	}

	i64 Context::getNBranches() {
