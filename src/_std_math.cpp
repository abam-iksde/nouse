	...
	Value* name(Context* ctx, i64 branch, i64 line, i64 fileind) { \
		...
		if (args.size() > 0) { \
			...
			if (args[0]->getType() == ValueType::FLOAT) { \
				...
				if (showErrors()) std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind) << "' line: " << line << ": first argument for '" << scrname << "' has to be an int or a float" << std::endl; \
			...
			if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": '" << scrname << "' takes one argument"; \
...
namespace nouse {
	...
	build1ArgMathFunction(_mathSin, "sin", std::sin)
	build1ArgMathFunction(_mathCos, "cos", std::cos)
	build1ArgMathFunction(_mathAbs, "abs", std::abs)
	...
	Value* _mathMax(Context* ctx, i64 branch, i64 line, i64 fileind) {
		...
		if (args.size() > 0) {
			...
			if (args[0]->getType() == ValueType::INT) {
				...
				if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": all arguments of 'max' have to be either int or float type";
				Value* result = new Value();
			...
			if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": 'max' takes at least one argument";
			Value* result = new Value();
			String* _s = new String("NotEnoughArguments");
			result->setError(_s);
			delete _s;
	...
	Value* _mathMin(Context* ctx, i64 branch, i64 line, i64 fileind) {
		...
		if (args.size() > 0) {
			...
			if (args[0]->getType() == ValueType::INT) {
				...
				if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": all arguments of 'min' have to be either int or float type";
				Value* result = new Value();
				String* _s = new String("InvalidArgument");
				result->setError(_s);
				delete _s;
			...
			if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": 'min' takes at least one argument";
			Value* result = new Value();
			String* _s = new String("NotEnoughArguments");
			result->setError(_s);
			delete _s;
	...
	Value* _mathRandomSeed(Context* ctx, i64 branch, i64 line, i64 fileind) {
		std::vector< Value* > args = ctx->getTopFunctionArgs();
		i64 seed = 0;
		if (args.size() < 1) seed = std::time(NULL);
		else {
			if (args[0]->getType() == ValueType::INT) {
				seed = args[0]->getInt();
			}
				...
				if (showErrors()) std::cout << "NOUSE ERROR: file: '" << *getSourceFileName(fileind) << "' line: " << line << ": the argument of 'random_seed' have to be either int or float type";
				Value* result = new Value();
				String* _s = new String("InvalidArgument");
				result->setError(_s);
				delete _s;
