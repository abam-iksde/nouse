...
Value* compValue(std::vector< Token >& tokens, i64& i) {
  ...
  /*else if (tokens[i].text == "this") {
          ...
          result = new Value();
          result->setFunctionCall(new FunctionCall(valueGetThis(), std::vector<
  Value* >(),tokens[i].line,tokens[i].file));
  }*/
  ...
  for (i64 i = 0; i < expr.size(); i++) {
    if (expr[i] != nullptr)
      if (expr[i]->getType() == ValueType::CALL_METHOD) {
        if (i - 1 >= 0 && i + 1 < expr.size() &&
            expr[i + 1]->getType() == ValueType::FUNCTION_CALL) {
          ...
          FunctionCall* fcPushThis =
              new FunctionCall(vPushThis, args, line, fileind);
          ...
          FunctionCall* fcObjectGetMethod =
              new FunctionCall(vnfObjectGet, args2, line, fileind);
          ...
          FunctionCall* fcDoWithThis =
              new FunctionCall(vDoWithThis, args1, line, fileind);
          ...
          if (showErrors())
            std::cout << "NOUSE COMPILATION ERROR file: '"
                      << *getSourceFileName(fileind) << "' line: " << line
                      << ": malformed method call" << std::endl;
        }
      }
  }
}
          ...
          if (op != nullptr) {
            ...
            case OperatorSides::BOTH: {
              ...
              if (i - 1 >= 0 && !isOperator(expr[i - 1])) {
                arg.push_back(
                    expr[i - 1] != nullptr ? expr[i - 1] : new Value());
                leftValid = b_true;
              } else
                arg.push_back(new Value());
              if (i + 1 < expr.size() && !isOperator(expr[i + 1])) {
                ...
                arg.push_back(new Value());
              FunctionCall* fc =
                  new FunctionCall(op, arg, line, fileind, b_false);
              delete expr[i];
              if (rightValid)
...
void scanLocalVarDeclaration(
    std::vector< Value* >& expr, i64 line, i64 fileind) {
    ...
    if (expr[j] != nullptr) {
      if (expr[j]->getType() == ValueType::NAME) {
        if (j + 1 >= expr.size()) {
          ...
          if (expr[j + 1] != nullptr &&
              expr[j + 1]->getType() == ValueType::NAME) {
            std::vector< Value* > args;
            Value* arg = new Value();
            ...
            args.push_back(arg);
            FunctionCall* fc =
                new FunctionCall(getVarCurrentScope(), args, line, fileind);
            delete expr[j];
            delete expr[j + 1];
            Value* v = new Value();
...
Function* compFunction(std::vector< Token >& tokens, i64& i) {
  ...
  if (expr.size() > 0) {
    scanFunctionCall(
        expr, tokens[tokens.size() - 1].line, tokens[tokens.size() - 1].file);
    Value* v = new Value();
    v->setNativeFunction(interpretTupleLiteral());
    FunctionCall* fcall = new FunctionCall(v, expr);
    Value* vv = new Value();
