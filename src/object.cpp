...
namespace nouse {
	...
	Value* _objectGet(Context* ctx, i64 branch,i64 line, i64 fileind) {
		...
		if (args[0]->getType() == ValueType::OBJECT) {
			if (args[1]->getType() == ValueType::STRING) {
				...
				if (showErrors()) std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind) << "' line: " << line << ": trying to index an object with a non-string value" << std::endl;
				Value* result = new Value();
			...
			if (showErrors()) std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind) << "' line: " << line << ": trying to index a value of type " << getValueTypeName(args[0]->getType()) << std::endl;
			Value* result = new Value();
	...
	Value* _thisGet(Context* ctx, i64 branch, i64 line, i64 fileind) {
		...
		if (_this->getType() == ValueType::OBJECT) {
			...
			if (showErrors()) std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind) << "' line: " << line << ": trying to index a value of type " << getValueTypeName(_this->getType()) << std::endl;
			Value* result = new Value();
			String* _s = new String("InvalidObject");
			result->setError(_s);
			delete _s;
	...
	void initObject() {
		...
		//fcGetThis = new FunctionCall(fvGetThis, std::vector< Value* >(), 0, b_false);
		//fcvGetThis = new Value();
		//fcvGetThis->setFunctionCall(fcGetThis);
	...
	stdstr_t Object::toString(Context* ctx,i64 line, i64 fileind) {
		if (ctx != nullptr && this->hasField("__str") && this->getField("__str")->getType() != ValueType::NONE) {
