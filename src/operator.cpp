...
i64 getOperatorBottomPriority() { return operators.size() - 1; }
Value* getOperator(stdstr_t s, i64 priority) {
  if (operators[priority].find(s) == operators[priority].end())
    return nullptr;
  return operators[priority][s];
}
OperatorSides getOperatorSides(stdstr_t s) {
  if (operatorsSides.find(s) == operatorsSides.end())
    return OperatorSides::NONE;
