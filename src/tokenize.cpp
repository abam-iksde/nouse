...
void tokenizeString(
    stdstr_t str, std::vector< Token >& tokens, stdstr_t filename) {
  i64 fileind = -1;
  if (filename == "unknown") {
  } else {
  ...
  tokens.push_back((Token){0, fileind, ";"});
  ...
  for (i64 i = 0; i < str.length(); i++) {
    if (inString) {
      if (str[i] == stringStart) {
        tokens.push_back((Token){line, fileind, buffer});
        buffer = "";
        inString = b_false;
      ...
      if (isWhitespace(str[i])) {
        ...
        if (chn_ == "/*") {
          ...
          tokens.push_back((Token){line, fileind, chn_});
          lastObjAccess = b_false;
        }
        // if (str[i] == '\\') lastObjAccess = b_true;
      } else if (str[i] == '.') {
        if (buffer.length() < 1) {
          tokens.push_back((Token){line, fileind, "."});
