  ...
  if (args.size() > 0) {
    if (args[0]->getType() == ValueType::INT) {
      ...
      std::vector< Value* >* list =
          _lists[_this->getObject()->getField(";listspot")->getInt()];
      delete _this;
      ...
      if (ind < 0 || ind >= list->size()) {
        if (showErrors())
          std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind)
                    << "' line: " << line << ": list index out of range"
                    << std::endl;
      ...
      if (showErrors())
        std::cout << "NOUSE ERROR file: '" << *getSourceFileName(fileind)
                  << "' line: " << line
                  << ": trying to index a list with a non integer value"
                  << std::endl;
...
Value* _listLength(Context* ctx, i64 branch, i64 line, i64 fileind) {
  ...
  Value* _this = objectGetThis();
  result->setInt(
      _lists[_this->getObject()->getField(";listspot")->getInt()]->size());
  delete _this;
  return result;
}
