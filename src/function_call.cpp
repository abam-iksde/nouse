...
namespace nouse {
	FunctionCall::FunctionCall(Value* f, std::vector< Value* > arg,i64 line_, i64 fileind_, boolean appendBranch_) {
		this->function = f;
		this->args = arg;
		this->line = line_;
	...
	Value* FunctionCall::execute(Context* ctx,i64 branch) {
		...
		if (f->getType() == ValueType::NAME) f = ctx->getVariable(f->getName(), branch-1);
		...
		for (int i = 0; i < args.size(); i++) {
			...
			if (a->getType() == ValueType::NAME) {
				...
				if (vFunction->getType() == ValueType::NATIVE_FUNCTION) {
					argVarRefs.push_back(!vFunction->getNativeFunction()->isResultDisposable());
				}
				else {
		...
		if (f->getType() == ValueType::FUNCTION) {
			...
			if (showErrors()) std::cout << "NOUSE ERROR file: '" << *getSourceFileName(this->fileind) << "' line: " << line << ": attepting to call a value of type " << getValueTypeName(f->getType()) << std::endl;
			result = new Value();
		...
		for (i64 i = 0; i < args_.size(); i++) {
			if (!argVarRefs[i]) delete args_[i];
		}
		return result;
	}
}
